...
async def api_incidents(auth: bool = Depends(check_auth)):
    ...
    cur.execute("""
        WITH status_changes AS (
            SELECT site, timestamp, status,
                   CASE WHEN status != 200 AND
                        (LAG(status) OVER (PARTITION BY site ORDER BY timestamp) = 200 OR
                         LAG(status) OVER (PARTITION BY site ORDER BY timestamp) IS NULL)
        ...
        SELECT site,
               ...
               CASE
                   WHEN MAX(status) = 0 THEN 'Timeout'
    ...
    return JSONResponse([
        {
            "site": r["site"],
            "start_time": r["start_time"].astimezone(TZ_MOSCOW).strftime("%d.%m %H:%M"),
            "dur": r["dur"],
...
def _build_html(data: dict) -> str:
    ...
    setPageProgress(25, 'Подготовка интерфейса');
    await new Promise(r => setTimeout(r, 120));

    setPageProgress(40, 'Загрузка аналитики');
...
async function loadIncidents() {{
    ...
    const loader = document.getElementById('incidents-loader');
...
@keyframes spin{
    ...
    <div class="page-loader-card">
        ...
        <div id="page-progress-text" class="page-progress-text">Загрузка страницы 0%</div>
    ...
    <div id="incidents-loader" class="section-loader" style="display:none;">
        <div class="loader-spinner small"></div>
        <div>Загрузка инцидентов...</div>
    </div>
    <div id="incidents-container">
        <div class="muted">Инциденты будут загружены при открытии вкладки.</div>
    </div>
</div>
...
if (name === 't2') loadCharts();
if (name === 't3') loadIncidents();
...
async function loadCharts(fromPageLoad = false) {
    if (chartsLoaded || chartsLoading) return;
    chartsLoading = true;

    const container = document.getElementById('charts-container');
    if (container && !container.innerHTML.trim()) {
        container.innerHTML = '<div class="section-loader"><div class="loader-spinner small"></div><div>Загрузка графиков...</div></div>';
    }

    try {
        if (fromPageLoad) setPageProgress(45, 'Запрос аналитики');

        const res = await fetch('/api/charts');

        if (fromPageLoad) setPageProgress(70, 'Обработка аналитики');

        const gdata = await res.json();

        if (fromPageLoad) setPageProgress(88, 'Отрисовка графиков');

        renderCharts(gdata);

        chartsLoaded = true;

        if (fromPageLoad) setPageProgress(95, 'Финализация');
    } catch (e) {
        if (container) {
            container.innerHTML = '<div style="padding:20px;color:#b91c1c;">Ошибка загрузки графиков</div>';
        }
    } finally {
        chartsLoading = false;
    }
...
function renderCharts(gdata) {
    const container = document.getElementById('charts-container');
    container.innerHTML = '';

    for (const [site, vals] of Object.entries(gdata)) {
        ...
        card.innerHTML = `
            ...
            <div class="chart-wrap">
                <canvas id="uptime_${safeSite}"></canvas>
            </div>
            <div class="chart-wrap">
                <canvas id="resp_${safeSite}"></canvas>
            </div>
        `;
        container.appendChild(card);

        new Chart(document.getElementById(`uptime_${safeSite}`), {
            type: 'line',
             {
                labels: vals.l,
                datasets: [{
        ...
        new Chart(document.getElementById(`resp_${safeSite}`), {
            type: 'line',
             {
                labels: vals.l,
                datasets: [{
