...
# career.sibur.ru добавлен в критичные со звездой
    ...
    while True:
        for site in SITES:
            try:
                ...
                if curr_status != 200:
                    fail_count[site] += 1
                    # Критичные - сразу, остальные - со 2-го раза
...
async def index(auth: bool = Depends(check_auth)):
    ...
    for s in sorted_sites:
        if s in g_data:
            html += f"<div class='kpi-card' style='border-top:2px solid #eee'><h5>{s}</h5><canvas id='c-{s.replace('.','_')}'></canvas></div>"
    ...
    for ev in sorted(cal_events, key=lambda x: x['d']):
        ...
        html += f"<tr><td>{ev['t']}</td><td>{ev['s']}</td><td class='{cls}'>{ev['d']} дн.</td></tr>"
    ...
    for s, d in g_data.items():
        html += f"""new Chart(document.getElementById('c-{s.replace('.','_')}'), {{
            type:'line', data:{{ labels:{json.dumps(d['l'])}, 
            datasets:[
                {{label:'Uptime %', data:{json.dumps(d['u'])}, borderColor:'#10b981', yAxisID:'y', tension:0.3}},
                {{label:'Ответ сек', data:{json.dumps(d['r'])}, borderColor:'#3b82f6', yAxisID:'y1', tension:0.3}}
            ]}},
